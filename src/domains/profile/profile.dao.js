...
import { getUser, getFitToUserId, getStyleToUserId, getFollowerToUserId, getFollowingToUserId,
    getMyFitSQL, getMyFitCategorySQL, getWishSQL, getWishCategorySQL, getBodyInfoSQL, getSetUpSQL } from "./profile.sql.js";
...
export const getSetUpDAO = async (userId) => {
    try {
        const conn = await pool.getConnection();
        const user = await pool.query(getSetUpSQL, userId);
        if(user[0].length == 0 ){
            conn.release();
            return -1;
        }
